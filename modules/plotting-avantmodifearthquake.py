...
FLOOD_BREAKS = [0.0, 0.3, 1.0, 2.0, np.inf]  # <=0.3, 0.3-1, 1-2, >2
...
def classify_flood_depth_array(depth_array: np.ndarray, nodata_value: float | int | None) -> np.ndarray:
    """
    Classify a depth raster into classes:
      0 = NoData/transparent
      1 = (0, 0.3]
      2 = (0.3, 1]
      3 = (1, 2]
      4 = (2, +inf)
    NaN are treated as NoData.
    """
    arr = depth_array.astype(float).copy()

    # NoData = declared nodata OR NaN (and keep <0 as NoData if nodata is unknown)
    if nodata_value is not None:
        nodata_mask = (arr == nodata_value) | np.isnan(arr)
    else:
        nodata_mask = np.isnan(arr) | (arr < 0)

    classes = np.zeros(arr.shape, dtype=np.uint8)  # 0 = NoData
...
def render_flood_raster_styled(
    ...
    """
    Standalone viewer for a single flood depth raster, using the classification above.
    """
    with rasterio.open(raster_path) as src:
        ...
        # Colormap: index 0 transparent (NoData), 1..4 with colors
        cmap = ListedColormap(["none"] + FLOOD_COLORS)
        bounds = [0, 1, 2, 3, 4, 5]  # 0..4 classes (5 edges)
...
def build_raster_legend_handles(spec, data_masked=None, vmin=None, vmax=None,  labels_override=None):
    ...
    """
    handles = []
    # use overridden labels if provided
...
def plot_and_save_exposure_map(
    ...
    if raster_path:
        with rasterio.open(raster_path) as src:
            nodata_val = src.nodata  # <-- keep nodata for flood classification
    ...
    if masked is not None:
        if hazard_name in {"pluvial_flood", "fluvial_flood"}:
            # >>> FLOOD special rendering (depth -> classes)
            ...
            ax.imshow(
                ...
                origin="upper",
                alpha=1.0,  # transparency handled by "none" for class 0
                zorder=1,
            )
            # Legend for flood
            legend_raster_handles = [
            ...
            # >>> Original behavior for other hazards
            if spec["type"] == "continuous":
                ...
                labels = list(spec.get("labels", ["Very low", "Low", "Medium", "High", "Very high"]))
                if spec.get("risk_direction") == "lower_is_worse":
                    labels = labels[::-1]
                legend_raster_handles = build_raster_legend_handles(
                    spec, data_masked=masked, vmin=vmin, vmax=vmax, labels_override=labels
                )
            else:
                cmap = ListedColormap(spec["palette"])
                norm = BoundaryNorm(spec["breaks"], len(spec["palette"]))
