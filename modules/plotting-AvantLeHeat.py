...
# ---- Seismic (PGA in g) shown as discrete classes in merged legend (no side colorbar) ----
...
def _seismic_palette(cmap_name: str = "plasma_r"):
    """Sample 4 ordered colors (light -> dark) from a continuous colormap for discrete bins."""
...
def build_raster_legend_handles(spec, data_masked=None, vmin=None, vmax=None,  labels_override=None):
    ...
    - For discrete rasters: use spec["palette"] + spec["labels"].
    - For continuous rasters: discretize into 5 bins and label Very low ... Very high.
    """
    handles = []
...
def plot_and_save_exposure_map(
    ...
    if (points is not None) and (len(points) > 0) and ("infra_type" in points.columns) and ("exposed" in points.columns):
        for pt_type in points[points["infra_type"] == pt_type]["infra_type"].unique() if len(points) > 0 else []:
            pass  # placeholder to avoid errors if structure changes
