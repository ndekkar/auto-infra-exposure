...
def render_flood_raster_styled(
    ...
    with rasterio.open(raster_path) as src:
        ...
        # Build colormap: index 0 transparent (NoData), 1..4 with colors
        ...
        im = ax.imshow(classes, extent=extent, origin="upper", cmap=cmap, norm=norm)
...
def plot_and_save_exposure_map(
    ...
    if masked is not None:
        if spec["type"] == "continuous":
            ...
            if spec.get("risk_direction") == "lower_is_worse": labels = labels[::-1]
            legend_raster_handles = build_raster_legend_handles(spec, data_masked=masked, vmin=vmin, vmax=vmax, labels_override=labels)
